    ...
    if (msg.quotedMessage.imageMessage) {
        ...
        ffmpeg(imagePath).outputOptions(["-y", "-vcodec libwebp"]).videoFilters('scale=2000:2000:flags=lanczos:force_original_aspect_ratio=decrease,format=rgba,pad=2000:2000:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1').save(imagePath2).on('end', async () => {
            ...
            if (msg.key.fromMe) {
                ...
                if (publicMessage) await sock.sendMessage(msg.key.remoteJid, { text: "_✅ Done!_", edit: publicMessage.key });
        ...
        if (msg.quotedMessage.stickerMessage.isAnimated) {
            ...
            if (animatedWebpToMp4 == "") {
                if (msg.key.fromMe) {
                    ...
                    if (publicMessage) await sock.sendMessage(msg.key.remoteJid, { text: "_❌ There is a error while converting animated sticker!_", edit: publicMessage.key });
                }
            } else {
                if (msg.key.fromMe) {
                    await sock.sendMessage(msg.key.remoteJid, { text: "_✅ Done!_", edit: msg.key });
